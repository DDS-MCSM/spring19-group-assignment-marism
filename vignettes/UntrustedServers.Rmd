...
De todos los resultados que hemos obtenido, los agrupamos per tipologia de servidor y mostramos los 20 primeros que contengan más resultados:
...
```
Filtramos por el tipo de servidor independientemente de la versión:
```{r main_servers, echo=FALSE, cache=TRUE}
servers.apache <- servers[grep("Apache", servers$x), ]
sum_apache <- sum(servers.apache$freq)
servers.nginx <- servers[grep("nginx", servers$x), ]
sum_nginx <- sum(servers.nginx$freq)
servers.win <- servers[grep("Microsoft", servers$x), ]
sum_win <- sum(servers.win$freq)
```
