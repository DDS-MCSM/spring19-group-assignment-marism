...
get.opendata.file <- function(url = NULL) {
  # Obtain raw data of opendata.rapid7.com website
  if (is.null(url)) { # Default file
  ...
  df.tcp21 <- read.csv(opendata.file, stringsAsFactors = FALSE)
  return(df.tcp21)
}

get.mixmind.file <- function() {
  ...
  unzip(zipfile = maxmind.file, exdir = file.path(getwd(), "data"), files = maxmind.source)
